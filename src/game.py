...
class Game():
    ...
    def apply_move(self, move: PlayerMove) -> None:
        ...
        if self.board.is_piece(move):
            sq: Piece = self.board.get_square(move)
        ...
        # Do move for empty square
            ...
            sq = None
            if move.is_gate():
                if move.get_type() == GateType.H:
                    sq = Gate(GateType.H)
                elif move.get_type() == GateType.NOT:
                    sq = Gate(GateType.NOT)
                elif move.get_type() == GateType.Z:
                    sq = Gate(GateType.Z)
            else:
